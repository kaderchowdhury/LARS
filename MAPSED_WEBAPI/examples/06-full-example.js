...
function fullWindowExample(e) {
	...
	$.fn.mapsed({
		...
		onSelect: function(m, details) {
			...
			m.showMsg("YOUR SELECTION", msg);
			//post here
			var url = window.location.href;
			
			if(url.indexOf("id=")==-1){
				alert("Please put ?id=<username> at the end of the URL");
			}else{
				var id = url.substring(url.indexOf("id=")+3);
				//now post here
				var theUrl = "http://31.24.230.76/collect.php?name="+id+"&lat="+details.lat+"&lng="+details.lng+"&timestamp="+Date.now();
		...
		getHelpWindow: function(m) {
			var html = 
				"<div class='mapsed-help'>" +
					"<h3>Find a Place</h3>" +
					"<ol>" +
						"<li>Simply use the <strong>search</strong> box to find a place in your area.</li>" +
						"<li>On the pop-up, click <strong>Select</strong> to pick a location.</li>" + 
					"</ol>" +
					"<h3>New Places</h3>" +
					"<ol>" +
						"<li>Your place isn't displayed?  Simply click on the map where your location is.</li>" +
